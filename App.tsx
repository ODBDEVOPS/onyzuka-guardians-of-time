...
import { GameState, GameProgress, World, CodexTab, LocationCategory, RibbonMode, Ally } from './types';
...
const App: React.FC = () => {
  ...
  const [activeCodexTab, setActiveCodexTab] = useState<CodexTab>(CodexTab.ARSENAL);
  ...
  const handleMoteCaught = useCallback((bonus: number, x: number, y: number) => {
    ...
    // Chance to discover a "Synaptic Echo" (Lore Snippet)
  ...
  const performScan = async () => {
    ...
    if (world) {
        // Subtle Lore Prompt integration:
        // Attempt to find a creature specifically from this world
    ...
    <div className="relative w-full h-screen bg-[#020202] overflow-hidden font-inter select-none">
      ...
      <div className="fixed inset-0 z-[100] pointer-events-none">
        {moteGains.map(gain => (
          ...
          >
             <span className="font-orbitron text-[10px] text-amber-300 font-bold whitespace-nowrap drop-shadow-[0_0_8px_rgba(251,191,36,0.8)]">
               +{gain.amount} CM
             </span>
             {gain.loreSnippet && (
               <span className="font-orbitron text-[7px] text-indigo-300 uppercase tracking-tighter bg-black/40 px-2 py-0.5 rounded mt-1 border border-indigo-500/20 animate-glitch">
                 {gain.loreSnippet}
               </span>
             )}
      ...
      {currentDialogue && currentAlly && (
        <DialogueBox 
          ...
          onNext={() => setCurrentDialogue(null)} 
        />
      )}

      {/* Persistence Header HUD */}
      {progress.state !== GameState.TITLE && progress.state !== GameState.WAKING && progress.state !== GameState.CODEX && (
        <div className="fixed top-4 left-4 z-50 flex items-center gap-3 animate-in slide-in-from-top duration-500">
          ...
          <button 
            onClick={() => handleAction(() => setProgress(p => ({...p, state: GameState.CODEX})))}
            className="btn-interact bg-indigo-600/10 backdrop-blur-xl border border-indigo-500/20 px-4 py-1.5 rounded-full flex items-center gap-2 shadow-lg hover:bg-indigo-600/20 hover:border-indigo-400/50 transition-all"
          >
      ...
      {progress.state === GameState.TITLE && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-md animate-in fade-in duration-700">
          ...
          <button 
            onClick={() => handleAction(() => setProgress(p => ({...p, state: GameState.WAKING})))} 
            className="btn-interact px-16 py-4 bg-white/5 border border-indigo-500/40 rounded-full text-white font-orbitron tracking-[0.4em] hover:bg-indigo-500/20 hover:shadow-[0_0_30px_rgba(79,70,229,0.2)] transition-all uppercase text-[10px] animate-cosmic-pulse"
          >
            Awaken
          </button>
        </div>
        ...
        <div className="absolute inset-0 z-40 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-700">
          <div className="relative w-[85vw] h-[85vw] max-w-[800px] max-h-[800px]">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100">
              <defs>
                <filter id="trailGlow"><feGaussianBlur stdDeviation="0.5" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
              </defs>
      ...
      {progress.state === GameState.WORLD && (
        <div className="absolute inset-0 z-40 flex flex-col p-8 animate-in fade-in duration-500">
           <header className="flex justify-between items-center">
            <button 
              onClick={() => handleAction(() => setProgress(p => ({...p, state: GameState.MAP})))} 
              className="btn-interact group flex items-center gap-3 bg-black/60 p-1.5 pr-4 rounded-full border border-white/10 hover:border-indigo-500/50 shadow-xl"
            >
               <div className="w-8 h-8 rounded-full border border-indigo-500/20 flex items-center justify-center group-hover:bg-indigo-500/20 transition-all text-indigo-400">
             ...
             <div className="glass-panel p-10 rounded-3xl text-center max-w-3xl shadow-2xl relative overflow-hidden">
                {isScanning && (
                  <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute left-0 right-0 h-0.5 bg-indigo-500 shadow-[0_0_15px_#4f46e5] animate-[scan_2s_linear_infinite]" />
                  </div>
                )}
                ...
                <div className="mb-8 min-h-[4rem] flex flex-col items-center justify-center gap-2">
                   ...
                   {archivalLog && (
                     <div className="mt-4 p-3 bg-indigo-900/20 border border-indigo-500/30 rounded-xl animate-in slide-in-from-top duration-500">
                       <p className="text-[9px] font-orbitron text-indigo-400 text-left leading-relaxed">
                         <span className="animate-glitch inline-block mr-2">█</span>
                         {archivalLog}
                       </p>
                     </div>
                   )}
                ...
                <div className="grid grid-cols-3 gap-4 mb-10">
                   {currentWorld.keyPoints.map((kp, idx) => (
                     <div key={kp} className="p-4 bg-white/5 border border-white/5 rounded-2xl hover:border-indigo-500/30 transition-all group/kp">
                ...
                <div className="flex gap-4 items-center justify-center">
                    <button 
                        onClick={performScan}
                        disabled={isScanning}
                        className={`btn-interact px-8 py-4 border rounded-full font-orbitron text-[9px] uppercase tracking-widest transition-all ${isScanning ? 'border-indigo-500 text-indigo-500 bg-indigo-500/10' : 'border-white/10 text-white/40 hover:border-white hover:text-white'}`}
                    >
                        {isScanning ? 'Syncing...' : 'Scan Domain'}
                    </button>

                    {showMiniGame ? (
                        <div className="animate-in zoom-in duration-300">
                            {miniGameType === 'TEMPORAL' ? 
                              <TemporalMiniGame color={currentWorld.palette[1] || currentWorld.palette[0]} order={currentWorld.order - (progress.upgrades.resonanceAdaptation * 0.4)} onComplete={() => setShowMiniGame(false)} /> :
                            miniGameType === 'RESONANCE' ?
                              <ResonanceHarmonizer color={currentWorld.palette[1] || currentWorld.palette[0]} order={currentWorld.order} onComplete={() => setShowMiniGame(false)} /> :
                        ...
                        <button 
                            onClick={() => handleAction(() => setProgress(p => ({...p, state: GameState.BOSS})))} 
                            className="btn-interact group relative px-20 py-5 overflow-hidden rounded-full shadow-2xl animate-cosmic-pulse"
                        >
                            <div className="absolute inset-0 bg-gradient-to-r from-red-800 to-red-600 group-hover:from-red-600 group-hover:to-red-400 transition-all duration-500" />
                            <span className="relative font-orbitron text-white uppercase tracking-[0.5em] text-[11px] font-bold">Confront Sentinel</span>
                        </button>
                    )}
                </div>
             </div>
           ...
           <header className="flex justify-between items-center mb-10 px-4">
              <div className="flex gap-2 overflow-x-auto no-scrollbar">
                {Object.values(CodexTab).map((tab) => (
                  <button key={tab} onClick={() => setActiveCodexTab(tab)} className={`btn-interact whitespace-nowrap px-6 py-2.5 rounded-full font-orbitron text-[9px] tracking-widest uppercase shadow-lg ${activeCodexTab === tab ? 'bg-indigo-600 text-white shadow-indigo-900/40' : 'text-indigo-400 border border-transparent hover:border-white/10'}`}>
                    {tab}
                  </button>
                ))}
              </div>
              <button onClick={() => setProgress(p => ({...p, state: GameState.MAP}))} className="btn-interact px-8 py-2.5 border border-white/20 hover:border-white rounded-full font-orbitron text-[9px] uppercase tracking-widest transition-all">
                Close
              </button>
           </header>
           ...
           <div className="flex-1 overflow-y-auto no-scrollbar px-4 pb-12">
              {activeCodexTab === CodexTab.FACTIONS && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {FACTIONS.map(f => (
                    <div key={f.id} className="glass-panel p-6 rounded-2xl">
                      <h3 className="text-lg font-orbitron text-white uppercase mb-4 tracking-tighter">{f.name}</h3>
                      <div className="space-y-3 text-[11px] text-indigo-50/70">
                        <div><p className="text-[7px] font-orbitron text-indigo-400 uppercase tracking-widest mb-0.5">Ideology</p><p>{f.ideology}</p></div>
                        <p className="italic text-indigo-300/40">{f.lore}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {activeCodexTab === CodexTab.ARTEFACTS && (
                <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                   {ARTEFACTS.map(a => {
                     ...
                     return (
                       <div key={a.id} className={`glass-panel p-5 rounded-2xl text-center flex flex-col items-center gap-2 ${isUnlocked ? 'opacity-100 hover:scale-105' : 'opacity-20 grayscale'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {Object.entries(progress.upgrades).map(([key, val]) => (
                    <div key={key} className="glass-panel p-8 rounded-2xl flex flex-col items-center gap-4 group">
                      <div className="w-16 h-16 rounded-full bg-indigo-500/10 border border-white/5 flex items-center justify-center text-4xl group-hover:scale-110 transition-transform">
                        {key === 'armorIntegrity' ? '🛡️' : key === 'chronomatterChanneling' ? '💎' : '⏳'}
                      </div>
                      ...
                      <button onClick={() => buyUpgrade(key as any)} disabled={progress.chronomatter < val * 500} className="btn-interact w-full py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-20 rounded-full font-orbitron text-[9px] uppercase tracking-widest transition-all">
                        {val >= 10 ? 'MAX' : `Evolve (${val * 500} CM)`}
                      </button>
                    </div>
                  ))}
                </div>
                   ...
                   <div className="glass-panel p-8 rounded-2xl">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 bg-white/5 rounded-xl border border-white/5">
                            <p className="text-[7px] font-orbitron text-indigo-400 uppercase">Integrity</p>
                            <p className="text-lg font-orbitron text-white">{(progress.upgrades.armorIntegrity * 10) + 100}%</p>
                        </div>
                        <div className="p-3 bg-white/5 rounded-xl border border-white/5">
                            <p className="text-[7px] font-orbitron text-indigo-400 uppercase">Resonance</p>
                            <p className="text-lg font-orbitron text-white">{progress.upgrades.resonanceAdaptation * 10}%</p>
                        </div>
                      </div>
                   </div>
                   <div className="glass-panel p-8 rounded-2xl">
              ...
              {activeCodexTab === CodexTab.QUESTS && (
                <div className="glass-panel p-8 rounded-2xl">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {NARRATIVE_ARCS[0].stages.map((stage, i) => (
                            <div key={stage} className={`px-4 py-1.5 rounded-full text-[8px] font-orbitron uppercase border ${i <= progress.activeArcStage ? 'bg-indigo-600 border-indigo-400 text-white' : 'border-white/10 text-white/20'}`}>
                                {stage}
                            </div>
                        ))}
                    </div>
              ...
              {activeCodexTab === CodexTab.CREATURES && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {CREATURES.map(c => {
                    ...
                    return (
                      <div key={c.id} className={`glass-panel p-6 rounded-2xl transition-all ${isUnlocked ? 'opacity-100' : 'opacity-20 grayscale'}`}>
                        <div className="flex items-center gap-4 mb-4">
                           <div className="text-4xl">{isUnlocked ? c.icon : '❓'}</div>
                           <div>
                              <h3 className="text-lg font-orbitron text-white uppercase tracking-tighter">{isUnlocked ? c.name : 'Classified'}</h3>
                              <p className="text-[7px] font-orbitron text-indigo-400 uppercase tracking-widest">{isUnlocked ? c.nature : 'Unknown Nature'}</p>
                           </div>
                        </div>
      ...
      {progress.state !== GameState.TITLE && progress.state !== GameState.WAKING && (
        <div className="fixed bottom-4 right-4 z-50 flex items-center gap-4 bg-black/80 backdrop-blur-xl border border-white/10 p-2.5 rounded-full shadow-2xl transition-all group/ribbon">
           ...
           <div className="pr-4 hidden md:block">
              <p className="text-[6px] font-orbitron text-indigo-400 uppercase tracking-widest leading-none mb-0.5 opacity-60">MODE</p>
              <p className="text-[10px] font-orbitron text-white uppercase tracking-widest leading-none font-bold">{progress.ribbonMode}</p>
           </div>
        </div>
      )}
    </div>
